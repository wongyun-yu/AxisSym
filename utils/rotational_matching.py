...
# Get 4 pemutation matrices list consists with 4 rotations 
def get_permutation_matrices(
    ...
    if disable_permutation:
        ...
        for i in range(num_rotations // 2):
            block = torch.eye(num_rotations).roll(-i, 0)
            blocks = [block for _ in range(int(num_channels * 2))]
            rotation_matrices.append(torch.block_diag(*blocks).to(device))

    return rotation_matrices


class RotationalMatching(nn.Module):
    def __init__(self, device, in_type, num_channels, feature_resolution, num_rotations, fix_seed):
        super(RotationalMatching, self).__init__()

        # Basic configurations
        self.device = device
        self.feature_resolution = feature_resolution
        self.num_rotations = num_rotations
        self.num_channels = num_channels[0] if isinstance(
            num_channels, list) else num_channels
    ...
    def _init_transformation_matrices(self) -> None:
        ...
        self.stacked_rotation_matrices = torch.stack(
            [self.permutation_matrices[int(i)]
    ...
    def _init_network_layers(self, in_type: FieldType) -> None:
        ...
        out_type = FieldType(
            self.gspace, [self.gspace.regular_repr] * self.num_channels)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # B, 4, (C x D_8), H*W]
        feature_init = torch.unsqueeze(input_tensor, dim=1).repeat(1, self.num_rotations // 2, 1, 1)
        # [B, num_rotations//2, num_channels, H*W]
        # [4, (C x D_8), (C x D_8)], [B, (C x D_8), H*W] -> [B, 4, (C x D_8), H*W]
        feature_rot = torch.einsum(
            "rmn, bns -> brms", self.stacked_rotation_matrices, input_tensor)

        # [B, 4, C, |D_8|, H, W]
        feature_init = feature_init.reshape(
            b, self.num_rotations // 2, self.num_channels, -1, h, w)
        # [B, 4, C, |D_8|, H, W]
        feature_rot = feature_rot.reshape(
            b, self.num_rotations // 2, self.num_channels, -1, h, w)

        # dim=3 cosine similarity
        cos_feat = self.cosine_similarity(feature_init, feature_rot)
        cos_feat = cos_feat.transpose(1, 2)  # [B, C, 4, H, W]
        cos_feat = cos_feat.reshape(b, -1, h, w)  # [B, C*4, H, W]
...
class RotationalMatchingGroup(nn.Module):
    ...
    def __init__(
        ...
        super(RotationalMatchingGroup, self).__init__()
    ...
    def _init_basic_configs(self, device, num_channels, feature_resolution,
        ...
        self.num_channels = num_channels[0] if len(
            num_channels) == 1 else num_channels
    ...
    def _init_multi_patch_layers(self, in_type: FieldType) -> None:
        ...
        out_types = [
            FieldType(self.gspace, [
                      self.gspace.regular_repr] * (self.num_channels[l]))
    ...
    # input: [B, C_in, H, W]
    def forward_single_patch(self, input: torch.Tensor):
        # [B, C_in, H, W]
        conv_features = self.relu(self.bn1(self.conv1(input)))
        ...
        unfolded_patches = self.unfold(projected_features.tensor).reshape(
            b, c, self.patch_size, self.patch_size, h, w)
        
        # [B*H*W, C*|D_8|, patch_size, patch_size]
        reshaped_patches = unfolded_patches.permute(
            0, 4, 5, 1, 2, 3).reshape(-1, c, self.patch_size, self.patch_size)
        
        # [B*H*W, C, |D_8|*patch_size*patch_size]
        init_patches = reshaped_patches.reshape(-1, self.num_channels, 2*self.num_rotations * self.patch_size * self.patch_size)
        init_patches = F.normalize(init_patches, p=2, dim=2, eps=1e-6)
        ...
        for i in range(self.num_rotations // 2):
            ...
            # [B*H*W, C, |D_8|*patch_size*patch_size]
            rotated_patches = F.normalize(
                rotated_patches, p=2, dim=2, eps=1e-6)
            ...
            similarity_map = torch.einsum(
                'bmc, bmc -> bm', rotated_patches, init_patches)
        ...
        # [B, 4C, H, W]
        similarity_maps = torch.cat(
            similarity_maps, dim=1)
        # [B, 4C, H, W]
        if self.feature_resolution == 64:
    ...
    def forward_multi_patch(self, input):  # input: [b, 64, H, W]
        ...
        for patch_idx in range(len(self.patch_size)):  # 'j' -> 'patch_idx'
            if not isinstance(self.patch_size, int):
                conv_features = self.relu(
                    self.bn1[patch_idx](self.conv1[patch_idx](input)))
                _, _, in_h, in_w = conv_features.shape
                if self.feature_resolution == 64:
                    conv_features = self.maxpool(conv_features)
                projected_features = self.conv2[patch_idx](
                    conv_features)  # [b, C|D_8|, H, W]
                b, c, h, w = projected_features.shape  # [b, C|D_8|, H, W]
            ...
            # [B*H*W, C, |D_8|*patch_size*patch_size]
            init_patches = reshaped_patches.reshape(-1, self.num_channels[patch_idx], 2*self.num_rotations * self.patch_size[patch_idx] * self.patch_size[patch_idx])
            init_patches = F.normalize(init_patches, p=2, dim=2, eps=1e-6)

            similarity_maps = []
            for rotation_idx in range(self.num_rotations // 2):
                ...
                rotated_patches = torch.einsum(
                    "lg, bghw -> blhw",
                    # indices updated
                    self.rotation_matrices[patch_idx][rotation_idx],
                    rotated_patches,
                )
            ...
            # [B, 4C, H, W]
            stacked_similarities = torch.cat(similarity_maps, dim=1)
            patch_outputs.append(stacked_similarities)

        # 'out_final' -> 'combined_similarity'
        combined_similarity = torch.cat(patch_outputs, dim=1)

        if self.feature_resolution == 64:
            combined_similarity = F.interpolate(
                combined_similarity, mode="bilinear", size=[in_h, in_w])
